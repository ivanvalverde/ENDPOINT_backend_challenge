...
let DirectoriesService = class DirectoriesService {
    operateDirectories() {
        const fileContent = (0, fs_1.readFileSync)('actions.txt', 'utf-8');
        const methods = ['CREATE ', 'MOVE ', 'DELETE ', 'LIST'];
        const directoriesStructure = [];
        let response = '';
        fileContent.split(/\r?\n/).forEach((line) => {
    ...
    create(methods, directoriesStructure, line) {
    ...
    move(methods, directoriesStructure, line) {
        ...
        dirChildrenToChange.forEach((dir) => {
            const parentDir = directoriesStructure.filter((parent) => parent.name === dir.parent)[0];
            dir.level = parentDir.level + 1;
            dir.relatives = parentDir.relatives.concat(parentDir.name);
        });
    }
    delete(methods, directoriesStructure, line) {
        ...
        const dirToDelete = directoriesStructure.filter((dir) => {
            ...
            return (JSON.stringify(dir.relatives) == JSON.stringify(parents) &&
        ...
        if (!dirToDelete) {
            ...
            while (i < dirToBeDeletedArr.length && breakOut === false) {
                const doesExist = directoriesStructure.filter((dir) => {
                    return (dir.name === dirToBeDeletedArr[i] &&
                        dir.level === i + 1 &&
            ...
            directoriesStructure.splice(directoriesStructure.indexOf(dirToDelete), directoriesStructure.indexOf(dirToDelete) - 1);
            const newDirectoriesStructure = directoriesStructure.filter((dir) => {
                return (dirToDelete === null || dirToDelete === void 0 ? void 0 : dirToDelete.name)
                    ? !dir.relatives.includes(dirToDelete === null || dirToDelete === void 0 ? void 0 : dirToDelete.name)
                    : true;
            });
        ...
        for (let i = 0; i < maxLevel; i++) {
            directoriesStructure.forEach((dir) => {
                if (dir.level === 1 && i == 0) {
                    ...
                    listResponse = listResponse.replace(`${dir.parent}\n`, `${dir.parent}\n${' '.repeat(dir.level)}${dir.name}\n`);
                }
            });
        }
        return listResponse;
